...
if [[ $PHP_VERSION == "8.0" ]]; then
  ...
  docker-php-ext-install -j "$(nproc)" exif pcntl bcmath bz2 calendar intl mysqli opcache pdo_mysql pdo_pgsql pgsql soap sockets memcache xsl zip gmp
  ...
  PHP_OPENSSL=yes docker-php-ext-configure imap --with-kerberos --with-imap-ssl
  docker-php-ext-install -j "$(nproc)" imap
  docker-php-ext-install -j "$(nproc)" exif xmlrpc pcntl bcmath bz2 calendar intl mysqli opcache pdo_mysql pdo_pgsql pgsql soap sockets memcache xsl zip gmp
  docker-php-ext-enable memcache
  docker-php-source delete
fi
